  ...
  try
    {
        ...
        *econfig=*ecfg;
        emploi.LoadFromFile(econfig->configfile);
...
void emEngine::Initialize(const bool AReset)
{
    ...
    for (tmpi=0 ; tmpi<MAX_SALLETYPE_COUNT; tmpi++){
       tmpf=emploi.SalleCountByType(tmpi);
    ...
    else
    {
        ...
        for (tmpi=0; tmpi<seances_count; tmpi++)
        {
            ...
            emploi.SeanceTable()[tmpi].doquinzwith=(int)foNoWhere;
            emploi.SeanceTable()[tmpi].dogroupwith=-1;
...
bool emEngine::StartSearching()
{
    ...
    while (iprocess<FProcessArrayLength)
    {
        ...
        #endif
                //emploi.getOptimizeValue(&valp,&valc,&osp,&osc);
...
bool emEngine::RempliSalle(bool abool)
{
    ...
    if (abool)
    {
          ...
          if (tmpgw!=-1)
          {
            emploi.setLink(tmpgw,cursindex,ltGroup);
          }
       ...
       // emploi.RempliSalle(cursindex,PrSalle,PrDay,PrHour,tmpfo,tmpgw,true);
       ...
       /* if (after_hc==true)
            prof_hc++;
        if((prof_hc>0)&&(PrEndhour!=8)&&(PrEndhour!=16)&&(emploi.ProfTable()[curpindex].weeka[PrDay][PrEndhour]!=-1))
            prof_hc--;
*/
        //dec delta
        //curSeance->delta--;
    ...
    else
    {
         ...
         */
        //emploi.RempliSalle(cursindex,PrSalle,PrDay,PrHour,tmpfo,tmpgw,false);
...
int emEngine::CheckIsEmpty()
{
    ...
    class_se=emploi.ClasseTable()[curcindex].weeka[PrDay][PrHour] ;//class  seance
    ...
    if (((iresult==-1)&& byquinz)|| ((iresult!=-1) && !byquinz))
    {
        ...
        class_se=emploi.ClasseTable()[curcindex].weekb[PrDay][PrHour] ;//class  seance
        iresultA=iresult;
        iresult=checkEmptyDT();
...
inline int emEngine::checkEmptyDT()
{
    ...
    bcangroup=emploi.CanBeByGroup(cursindex,class_se,PrDay,PrHour);
...
void emEngine::NextDay()
{
   ...
   // after_hc=(PrHour>0) && (PrHour!=8) && (emploi.ProfTable()[curpindex].weeka[PrDay][PrHour-1]==-1);
    isForbiddenday=
        (PrDay==curfday)
...
*/
        ...
        //heure creuse
      //  || ( (iprocess<lowlevel_process_count)&& ((PrHour>0) && (emploi.ClasseTable()[curcindex].weeka[PrDay][PrHour-1]==-1))  )
      //  || ( (prof_hc>10)&& after_hc  )
        ;
