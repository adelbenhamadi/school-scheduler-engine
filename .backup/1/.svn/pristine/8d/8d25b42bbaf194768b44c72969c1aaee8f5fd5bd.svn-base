...
int main(int argc, char *argv[], char *envp[])
{
    ...
    if(justCheck==true){
        ...
        engine->emploi.rebuildSolution();
        printf("rebuild solution..[ok]\n\n");
        engine->emploi.getOptimizeValue(&valp,&valc,&osp,&osc);
        wsprintf(str1,"->solution optimize info:%d %d/%d/%d/%d\n",sol,valp,valc,osp,osc);
        Console::Write(str1);


        if (engine->emploi.verifyProcessedSeances(false)==false)
            {
            ...
            else
            {
                ...
                a_percent=(double) (100*(1-(val/engine->emploi.SeanceTableCount()))) ;
