...
void emEngine::Initialize(const bool AReset)
{
  ...
  /*  if (getProcessArrayChecksum()==FProcessArrayChecksum){
        doRandomize();
        doSort(true);
        Console::WriteEx("\nduplicated search checksum reprocessing..\n",ColorRed);
        //process_tries_count=0;
        process_last_progession=process_tries_count;
        max_processed_seances=0;
        //time(&ptime);
...
inline int emEngine::checkEmptyDT()
{
   ...
   // pse=(*ProfDT)[PrDay][PrHour] ; //prof  seance
    ...
    bcangroup=(class_se!=-1)&&(emploi.CanBeByGroup(cursindex,class_se,PrHour));
...
FORCEINLINE void emEngine::NextDay()
{
    ...
    isForbiddenday=
            ...
            ||(  (curmindex==4) && (FMatProcessMap[4].iprocess<FMatProcessMap[4].level) && (PrEndhour>8) && (PrDay>=4)    )
